    ...
    def step(self):
        if self.resource_quantity:
            ...
            while adjust_round < self._adjust_rounds:
                remaining_resource = max(.0, self.resource_quantity * (1.0 - sum([x for x in agent_shares.values() if x])))
                    ...
                    if round_finished:
                        ...
                        for agent in self.commons_agents:
                            adjustment_list = list(filter(lambda x: x != 0, [act.consumption_adjustment[agent.id]
                                                    for act in agent_actions.values() if act.consumption_adjustment]))
                            if adjustment_list:
